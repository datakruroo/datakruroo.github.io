...
calc_summary_stats <- function(sim_data) {
  sim_data %>%
    select(learning_performance, ach, learning_performance_mar) %>%
...
อีกวิธีการหนึ่งคือการใช้โมเดลการวิเคราะห์ความลำเอียงในการเลือกตัวอย่าง (sample selection bias) หรือ Heckman Selection Model หรือ Heckman Correction [@Galimard2018] 
...
จากรูปด้านล่างพอเห็นว่ามีความสัมพันธ์กันระหว่างการสูญหายใน learning performance กับ ach โดยมีความสัมพันธ์ในทิศทางลบ กล่าวคือการสูญหายมีแนวโน้มพบมากในกลุ่มนักเรียนที่มี ach ต่ำ อย่างไรก็ตามการสร้างสมการทำนายเชิงเส้นจากความสัมพันธ์ใน scatter plot ด้านล่างนั้นไม่ค่อยเหมาะสม

```{r}
glimpse(data)
data |> 
  select(ach, learning_performance_mar, engage, ontime_class, practice) |> 
...
- การใช้โมเดลสถิติขั้นสูง: เช่น การใช้ Selection models หรือ Pattern-mixture models ที่ถูกออกแบบมาเพื่อตรวจสอบกลไกการสูญหายของข้อมูลแบบ MNAR โดยจะรวมตัวแปรที่เกี่ยวข้องกับการสูญหายมาในโมเดลด้วย เพื่อสร้างความเป็นไปได้ในการคาดการณ์ค่าที่สูญหาย
...
data |> 
  mutate(learning_performance_mean_imp =
          ifelse(is.na(learning_performance_mar)==T,
                ...
                learning_performance_mar)) |> 
  select(learning_performance, learning_performance_mean_imp) |> 
...
dat_imp_mean |> select(learning_performance_mcar, learning_performance_mcar_imp) |> 
