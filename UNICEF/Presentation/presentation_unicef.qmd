...
library(tidyverse)
library(gt)
load("/Users/choat/Documents/GitHub/unicef/Presentation/cartogram.RData")
```
...
- Y-Axis: Shows the indicators used for the analysis

- X-Axis (Horizontal Axis): Shows the loading values for each indicator, which explain how strongly each variable contributes to each Principal Component (PC). Values closer to -1 or 1 indicate stronger correlations, while values close to 0 suggest weak or no correlation.

- Each Block (PC1 - PC4): Represents the different Principal Components. For each component, reader can see how each variable is related to it. 

- Color of the Bars: Represents the direction of the correlation, red color for negative correlation (values below 0), and blue color for positive correlation (value above 0). A positive value (blue) indicates that as the indicator increases, the Principal Component increases. A negative value (red) means that as the indicator increases, the Principal Component decreases.
...
<div class="caption">fig 3.6: Schools with high CMER levels over the 22 river basins map (2017-2021).</div>
...
```{r megeing_data_for_map, echo = F, eval = F}
...
- The analysis found that **schools lacking readiness in infrastructure and connectivity tend to have higher risks compared to schools that are fully or partially ready. This difference is particularly evident in schools located in remote areas.“​**
...
- **School Characteristics (2 variables):**
  ...
  - School Size: Categorized into four sizes—small, medium, large, and extra-large. The size of a school is a significant factor that can reflect the resources and budget allocated for educational management.
  
  - Student Level: Based on the highest grade offered at the school, divided into three levels—Grade 6 (Primary School), Grade 9 (Middle School), and Grade 12 (High School).

- **Student Socioeconomic Status (1 variable):**

  - Percentage of Low-Income Students: This factor is available only for schools under the PESA. Before analysis, the researchers converted this variable into a categorical variable with two levels: (1) Schools where the majority of students are from low-income families (percent of low-income student > 50%), and (2) Schools where low-income students are a minority. This conversion was made to ensure that the analysis results are clearly interpretable and communicable.​
...
collapsed_data <- tidy_results |> 
  ...
  mutate(term = ifelse(str_detect(term, "^student_level_"), "Student Level", term)) %>%  # ยุบชื่อ school_size_XXX ให้เป็น school_size
  ...
  summarise(
    vip = sum(vip),  # รวมค่า vip
    percent_vip = max(percent_vip),  # รวมค่า percent_vip
    r_squared = max(r_squared)  # เก็บค่า r_squared ที่มากที่สุด
  ) %>%
  ungroup()
...
collapsed_data |> 
  ...
  ungroup() |> 
  mutate(area = ifelse(str_detect(affiliation,"สพป."),"PESA", "SESA")) |> 
  ...
  xlab("\n R Squared")+
  theme_light()+
  ggtitle("R-squared for the models of each educational service area.”")+
...
```{r collapsed_data_creating, eval = F}
collapsed_data <- tidy_results |> 
  unnest(c(data)) |> 
  select(province, amphor,affiliation, term, vip,r_squared ) |> 
  mutate(percent_vip = vip*100/max(vip)) |> 
...
# กำหนดระดับของ term ตามลำดับที่จัดเรียง
...
```{r generate_cartogram, eval = F}
school_sf_merc <- st_transform(school_sf |> st_as_sf() , 3857)
school_sf_merc_cartogram <- cartogram_cont(school_sf_merc, "vip", itermax = 5)

school_sf_cartogram <- st_transform(school_sf_merc_cartogram, st_crs(school_sf |> st_as_sf()))
plot(st_geometry(school_sf_cartogram))
```

```{r fig.height = 14, fig.width = 20}
school_sf_label_carto <- school_sf_cartogram |> ungroup() |> 
  filter(str_detect(affiliation, "สพป.")) |> 
  group_by(term) |> 
  mutate(relative = vip*100/(max(vip))) |> 
  ungroup() |> 
  filter(relative > 50) |> 
...
cartogram <- ggplot(school_sf_cartogram) +
  geom_sf(aes(fill = term), linewidth = 0, alpha = 0.9) +
  geom_sf_text(data = school_sf_label_carto, aes(geometry = geometry, label = affiliation), size = 2, color = "grey20", check_overlap = TRUE,
  family = "ChulaCharasNew")+
  theme_minimal() +
  ...
  #                      guide = "none")+
  ggtitle("The most importance factor associated \n with the CMER Index in each PESA.",
          subtitle = "Cartogram Representation")


school_sf_label <- school_sf |> ungroup() |> 
  filter(str_detect(affiliation, "สพป.")) |> 
  group_by(term) |> 
  mutate(relative = vip*100/(max(vip))) |> 
  ungroup() |> 
...
school_sf <- collapsed_data |> 
  ...
  group_by(province, affiliation) |> 
  slice_max(percent_vip) |> 
  inner_join(shapefile_data, by = c("province" = "ADM1_TH"))  # เปลี่ยน join_by 
...
sesa_p <- ggplot(data = thailand_map) +
  ...
  ggtitle("The most importance factor associated \n with the CMER Index in each SESA.")

PESA_p + cartogram + sesa_p
```
...
## Risk-Importance matrix {.smaller}
...
- Y-axis: Displays the CMER Index, representing educational risk derived from academic achievement and extreme rainfall conditions. Higher values indicate greater risk levels that demand attention.

- X-axis: Represents the Relative Importance of six factors affecting educational risk, calculated using variable importance (VIP). This axis shows how significantly each factor contributes to the overall risk in each district.
...
- **Strategic Focus:** Areas with high risk and high importance of contributing factors. These areas require immediate policy attention and intervention.

- **Development Priority:** Areas with moderate to high risk but where factors have lower importance or influence on the overall risk, yet targeted development efforts can effectively reduce potential negative impacts and improve overall resilience.

- **Monitoring:** Areas with lower risk but higher factor importance, which should be monitored to ensure risks do not escalate.

- **Low-risk Group:** Areas with both low risk and low factor importance, which may require minimal intervention but regular monitoring.

</div>


## Risk-Importance matrix (RIM): PESA {.smaller}
...
```{r}
 data_for_performance <- collapsed_data |> 
  mutate(group = case_when(
    str_detect(affiliation, "สพป.") ~ "PESA",
    str_detect(affiliation, "สพม.") ~ "SESA"
  )) |> 
  inner_join(
  precipitaion_risk_data_cleaned_aggregated |> 
    group_by(affiliation) |> 
...
# กำหนดจุดที่ต้องการให้เส้นผ่าน
x_points <- c(10, 50, 105)
y_points <- c(0.70, 0.6, 0.48)
spline_curve <- spline(x_points, y_points, n = 200)
...
# สมมุติว่า data_for_performance มีคอลัมน์ x (percent_vip) และ y (CMER)
...
# กรองเฉพาะข้อมูลที่ CMER สูงกว่าเส้น spline
...
data_for_performance |> 
  ...
  geom_line(data = spline_curve, col = "maroon",
    aes(x = x, y = y), linetype = 3) +
  geom_line(data = spline_curve2,
    aes(x = x, y = y), linetype = 3) +
   geom_segment(aes(x = 0, y = 0.6, xend = 100, yend = 0.2), linetype = 3) +  
...
geom_text_repel(data = data_for_performance2 %>%
 ...
 geom_label(x=80, y=0.77, label = "Strategic Focus",
  family = "ChulaCharasNew", col = "maroon")+
  geom_label(x=87, y=0.28, label = "Development Priority",
  angle = -25,
  family = "ChulaCharasNew")+
  ...
  family = "ChulaCharasNew")+
    ...
    xlim(0,100)+
    xlab("\n Relative Importance (%)") +
    ylab("\n Climate-Map Educational Risk (CMER) \n")+
    theme_light()+
    theme(panel.grid = element_blank(),
...
## Risk-Importance matrix (RIM): SESA {.smaller}
...
viewof selected_affiliation = Inputs.select(
  Array.from(new Set(collapsed_data.map(d => d.affiliation))).sort(),
  {
    ...
    element: (input) => {
      // ปรับขนาดฟอนต์ของ select และ label
...
chart = {
  if (filtered_data.length > 0) {
    ...
    // จัดเรียงข้อมูล filtered_data จากมากไปน้อยตาม percent_vip
    const sorted_data = filtered_data.slice().sort((a, b) => b.percent_vip - a.percent_vip);

    // ดึงค่าของ term ที่จัดเรียงแล้วเพื่อใช้เป็น domain ในแกน y
    ...
    return Plot.plot({
      ...
      x: { label: "Percent VIP (%)" },
      y: { label: "Non-Climate Factor \n \n \n", domain: sorted_terms },  // กำหนดลำดับของแกน y ตาม terms ที่จัดเรียงแล้ว
      height: 400,
      width: 600,
