...
- Student Socioeconomic Status (1 variable):
  ...
  - Percentage of Low-Income Students: This factor is available only for schools under the Office of the Basic Education Commission (OBEC). Before analysis, the researchers converted this variable into a categorical variable with two levels:
  
  - Schools where the majority of students are from low-income families (percent > 50%).
  
  - Schools where low-income students are a minority.
This conversion was made to ensure that the analysis results are clearly interpretable and communicable.​

Before processing, the researchers prepared the data for analysis by cleaning it and imputing missing values using the bagging trees algorithm.
...
PESA_p <- ggplot(data = thailand_map) +
  geom_sf() +
  geom_sf(data = school_sf, aes(geometry = geometry, fill = term),
          alpha = 0.5, size = 0.8, color = NA) +  
  geom_sf(data = affiliation_boundaries, aes(geometry = geometry), 
          color = "black", fill = NA, size = 1, show.legend = FALSE) +  # ตีเส้นขอบตาม affiliation
  geom_sf_text(data = school_sf_label, aes(geometry = geometry, label = affiliation), size = 2, color = "grey20", check_overlap = TRUE,
  family = "ChulaCharasNew")+
...
The figure below is called a variable importance plot. Readers can select and view the factors associated with the CMER Index for each educational service area.
