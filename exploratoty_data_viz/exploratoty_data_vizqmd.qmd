...
data2 <- data %>% 
  ...
  select(Department, basic_concept, Concepts, Interpret, ChooseMethod,
         ...
         total) %>% 
  rename(StatKnowledge = basic_concept,
         ResFinalScore = total ,
...
use_data %>% 
  ggplot(aes(x = StatKnowledge))+
  geom_histogram(aes(fill = StatKnowledge > 50, y = after_stat(count)), 
                 bins = 20,
                 col = "white", alpha = 0.6)+
  ...
  theme_light()+
  scale_fill_discrete(name = "StatKnowledge Level" ,labels = c("Fail","Pass"))+
  theme(text = element_text(family = "ChulaCharasNew", size = 16),
        legend.position = "top",
        panel.grid = element_blank())
```
...
```{r echo = F, fig.width=5, fig.height = 4}
use_data %>% 
  ggplot(aes(x = StatKnowledge > 50, fill = StatKnowledge > 50))+
  geom_bar(alpha = 0.6)+
  geom_text(stat = "count", aes(label = ..count..), size = 8, vjust = - 0.5,
            family = "ChulaCharasNew")+
  theme_light()+
  ylim(0,300)+
  scale_x_discrete(name = "StatKnowledge", labels = c("Fail","Pass"))+
  theme(text = element_text(family = "ChulaCharasNew", size = 16),
...
use_data %>%
  mutate(StatKnowledge_Group = cut(
    StatKnowledge, 
    breaks = c(-Inf, 50, 75, Inf), 
    labels = c("Fail", "Pass", "Good")
  )) %>%
  ggplot(aes(x = StatKnowledge)) +
  geom_histogram(aes(fill = StatKnowledge_Group, y = after_stat(count)), 
                 bins = 20, 
                 col = "white", alpha = 0.6) +
  ...
  theme_light() +
  scale_fill_discrete(name = "StatKnowledge Level")+
  theme(
...
p2 <- use_data %>%
  ...
  ggplot(aes(x = StatKnowledge_Group)) +
  geom_bar(aes(fill = StatKnowledge_Group), alpha = 0.7)+
  geom_text(stat = "count", aes(label = ..count..), size = 3, vjust = - 0.5,
            family = "ChulaCharasNew")+
  theme_light()+
  ylim(0,300)+
  scale_x_discrete(name = "StatKnowledge", labels = c("Fail","Pass","Good"))+
  theme(text = element_text(family = "ChulaCharasNew"),
        legend.position = "none",
        panel.grid = element_blank())+
...
```{r}
use_data %>% 
  select(StatKnowledge, StatSubmitTime, StatPercentSubmit, StatLearnPerform, StatCheatBehav) %>% 
  pivot_longer(cols = -(StatKnowledge)) %>% 
  ggplot(aes(x = value, fill = StatKnowledge>50))+
...
p1 <- use_data %>% 
  select(StatKnowledge, StatSubmitTime, StatPercentSubmit, StatLearnPerform, StatCheatBehav) %>% 
  ggplot(aes(x = StatCheatBehav, fill = StatKnowledge > 50)) +
...
use_data %>% 
   mutate(StatKnowledge_group = cut(StatKnowledge,
                                   breaks = c(-Inf, 50, Inf), 
                                   labels = c("Fail","Pass"))) %>% 
   group_by(StatKnowledge_group) %>% 
...
use_data %>% 
  select(StatKnowledge, StatSubmitTime, StatPercentSubmit, StatLearnPerform, StatCheatBehav) %>% 
  pivot_longer(cols = -(StatKnowledge)) %>% 
  ggplot(aes(x = value, fill = StatKnowledge>50, y= StatKnowledge>50))+
  #geom_violin()+
...
```{r}
use_data  %>%
  ggplot(aes(x = StatKnowledge, y= Department))+
  #geom_violin()+
...
```{r}
use_data  %>%
  ggplot(aes(x = StatKnowledge, y= reorder(Department,StatKnowledge, FUN = median)))+
  #geom_violin()+
  geom_boxplot(alpha = 0.5, fill = "steelblue", alpha = 0.7)+
  theme_light()+
  theme(legend.position = "none")+
...
p1 <- use_data %>%
  mutate(Department = fct_reorder(Department, 
                                  -StatKnowledge > 50, # จัดลำดับโดยค่าที่ FALSE
                                  .fun = function(x) sum(!x))) %>%
  ggplot(aes(fill = StatKnowledge > 50, y = Department)) +
  geom_bar(position = "stack") + 
  theme_light() +
  xlab("\n StatKnowledge") +
  ylab("") +
  scale_fill_discrete(name = "StatKnowledge",labels=c("Fail","Pass"))+
  theme(
    legend.position = "top",
    text = element_text(family = "ChulaCharasNew", size = 16)
  )+
...
filtered_data <- use_data %>%
  filter(StatKnowledge <= 50) %>%
  arrange(desc(StatKnowledge)) %>% 
...
use_data <- data %>% 
  ...
  rename(StatKnowledge = basic_concept,
         ...
         StatCheatBehav = CheatBehavior) %>% 
  mutate(Student_id = 1:dim(use_data)[1], .before = "Department") %>% 
  select(Student_id:StatCheatBehav, ResMidterm) %>% 
  rename(StatKnowledge = 3)

use_data %>% 
  ggplot(aes(x = StatKnowledge, y = ResMidterm))+
  geom_point()+
  theme_light()
```
...
p2 <- use_data %>% 
  ...
  ggplot(aes(x = SubmitTime, y=StatKnowledge))+
  geom_boxplot()
...
p1 <- use_data %>% 
  ...
  scale_x_discrete(labels = c("<=75 ชั่วโมง",
                              "75-125 ชั่วโมง",
                              "125-168 ชั่วโมง",
                              "> 168 ชั่วโมง"))
...
## Feedback กิจกรรมในชั้นเรียน 1 {.smaller}
...
hw %>% 
  ...
  mutate(answer = fct_relevel(answer, "ไม่ควร")) %>% 
  ggplot(aes(x = "", y= n))+
  geom_bar(stat = "identity", aes(fill = answer), col = "grey20")+
  geom_text(aes(label = paste0(answer," = ",n, "คำตอบ \n (",round(percent,2),"%)")), 
            position = position_stack(vjust = c(0.6,0.5)), 
            size = 5.5, color = "white", family = "ChulaCharasNew",
            fontface = "bold") +
  scale_fill_manual(values = rev(c("#DDA853","#16404D")))+
  coord_polar(theta = "y")+
  theme_void()+
...
## Feedback กิจกรรมในชั้นเรียน 2 {.smaller}
...
## Feedback กิจกรรมในชั้นเรียน 3 {.smaller}
...
# โหลด Python module pythainlp
py <- import("pythainlp")

# ตัดคำภาษาไทย
text <- "นิสิตส่วนมากสามารถส่งงานได้ตรงเวลา"
tokens <- py$word_tokenize(text)
print(tokens)
```
...
tokenizer <- transformers$AutoTokenizer$from_pretrained("airesearch/wangchanberta-base-att-spm-uncased")

tokens <- hw[1:3,6] %>% 
  mutate(tokens = map(reason, ~ tokenizer$tokenize(as.character(.x))))


# ตัดคำ
text <- as.character(hw[1,6])
...
```{r eval = F}
reason_tokens <- hw %>% 
  mutate(tokens = map(reason, 
                      ~ tokenizer$tokenize(as.character(.x)))) %>% 
  unnest(tokens) %>% 
  rename(word = tokens)

reason_tokens %>% 
  filter(answer == "ไม่ควร") %>% 
  mutate(word = str_remove(word, "^▁")) %>% 
  mutate(word = str_remove(word, "▁")) %>% 
  filter(word != "") %>% 
  filter(!str_detect(word, "^[a-zA-Z]$")) %>% 
  count(answer, student_id, word) %>% 
  bind_tf_idf(term = "word", document = "student_id", n) %>% 
  arrange(desc(tf_idf))
```
...
dtm <- reason_tokens %>% 
  filter(answer == "ไม่ควร") %>% 
  mutate(word = str_remove(word, "^▁")) %>%   # ลบ "▁" ที่ขึ้นต้นคำ
  mutate(word = str_remove(word, "▁")) %>%
  filter(word != "") %>% 
  filter(!str_detect(word, "^[a-zA-Z]$")) %>%   # ลบตัวอักษรภาษาอังกฤษเดี่ยว ๆ
  count(student_id, word) %>% # นับจำนวนคำต่อเอกสาร
  cast_dtm(document = student_id, term = word, value = n)

dtm
```

```{r eval = F}
# กำหนดจำนวน Topic (เช่น 3 topic)
k <- 4
