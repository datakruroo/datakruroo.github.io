...
3.  `cost_complexity`: ค่า penalty ที่ใช้ชดเชย impurity หรือ Total SSE ของ decision tree การทำ penalty ใน tree จะช่วยลดความเสี่ยงที่จะเกิด overfitting
...
วัตถุประสงค์คือ การสร้างโมเดลทำนายคะแนนสอบ midterm รายวิชาวิจัยของนักเรียนโดยใช้ตัวแปรทำนายได้แก่ คะแนนที่เกี่ยวข้องกับความรู้ทางสถิติ และพฤติกรรมการเรียนทั้งหลายของนักเรียน

```{r}
library(tidyverse)
data <- read_csv("/Users/choat/Library/CloudStorage/OneDrive-ChulalongkornUniversity/Documents_backup_mid2025/ภาควิชา/learning_data.csv")
data <- data %>% select(-student_id)
data <- data %>% 
  mutate(student_id = 1:385, .before = 1)
glimpse(data)
```

### 5.4.1 Classification Tree

classification คือการจำแนก observation ออกเป็นกลุ่มด้วยโมเดลทำนาย ดังนั้นเราจะต้องสร้างตัวแปร y เชิงกลุ่มตามที่ต้องการ

```{r}
data %>% 
  mutate(res_class2 = ifelse(research_score >= 70, "success", "not_success")) %>% 
  ggplot(aes(x = res_class2)) +
  geom_bar()
```
...
```{r}
data %>% 
  mutate(res_class3 = case_when(
    research_score >= 80 ~ "A",
    research_score >= 70 & research_score < 80 ~ "B",
    research_score < 70 ~ "C"
  )) %>% 
  ggplot(aes(x = res_class3)) +
  geom_bar()
```
...
```{r}
set.seed(123)
split <- initial_split(full_data, prop = 0.8, strata = res_class3)
...
```{r}
rec <- recipe(res_class3 ~ ., data = train_data) %>% 
  update_role(student_id, new_role = "id") %>% 
  step_rm(department, gender, research_score, res_class2) %>% 
  step_impute_bag(cheat_index)
```
...
```{r}
set.seed(123)
folds <- vfold_cv(train_data , v=5, repeats = 3, strata = res_class3)
...
```{r}
dt_res %>% show_best(metric = "roc_auc", n = 5)
```



จากผลการคัดกรองเบื้องต้นด้วยดัชนีหลักคือ roc auc เราเลือกมา 2 โมเดลก่อน ได้แก่
...
```{r}
dt_res %>% collect_metrics() %>% 
  filter(.config %in% c("pre0_mod08_post0", "pre0_mod24_post0")) %>% 
...
ลองหา confusion matrix

```{r}
dt_res %>% collect_predictions() %>% 
  filter(.config == "pre0_mod08_post0") %>%
  conf_mat(truth = res_class3, estimate = .pred_class)

dt_res %>% collect_predictions() %>% 
  filter(.config == "pre0_mod24_post0") %>%
  conf_mat(truth = res_class3, estimate = .pred_class)
```
...
dt_res %>% collect_predictions() %>% 
  filter(.config == "pre0_mod08_post0") %>% 
  group_by(res_class3) %>% 
  sens(truth = res_class3, estimate = .pred_class)
```



ลอง plot ต้นไม้ทั้งสองต้นดู
...
การจะใช้ rpart.plot ร่วมกับ tidymodels เราจะเป็นต้องดึงเฉพาะผลการวิเคราะห์/ประมวลผล ที่เป็น rpart object ออกไปให้ rpart.plot เท่านั้น 

ก่อนที่จะเอามาสร้างแผนภาพ เราจำเป็นต้องเทรนโมเดลที่สนใจจากชุดข้อมูลฝึกหัดก่อน แล้วถึงเอามา plot
...
```{r}
my_model1 <- dt_wf %>% 
  finalize_workflow(dt_res %>% select_best(metric = "roc_auc")) %>% 
  fit(train_data)

my_model2 <- dt_wf %>% 
    finalize_workflow(dt_res %>% show_best(metric = "roc_auc", n =5) %>% slice(4)) %>% 
  fit(train_data)
...
ก่อนไป plot เราเลยจำเป็นต้องแปลง workflow ให้เป็น rpart object ก่อน ดังนี้
