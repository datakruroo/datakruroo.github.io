...
-   Random forest มีการควบคุมเพื่อหลีกเลี่ยงปัญหา redundancy ด้วยการ

    1.  จากชุดข้อมูล training data ที่มีตัวแปรอิสระจำนวน p ตัว
    2.  random forest จะให้เรากำหนด `mtry` คือจำนวนตัวแปรอิสระที่จะสุ่มมาเพื่อสร้าง weak learning ในแต่ละรอบ
    3.  กำหนดจำนวนต้นไม้ `trees` ที่จะใช้ในการเรียนรู้ โดยที่จำนวนต้นไม้นี้ก็จะมีจำนวนเท่ากับ boostrap sample
    4.  เราสามารถควบคุมการเรียนรู้ของต้นไม้ (strump) แต่ละต้นให้เรียนรู้แบบพอรู้ได้ โดยใช้ `min_n`
...
สร้าง data preprocessing model

```{r}
rec <- recipe(G3 ~. ,data = train_data) %>% 
...
กำหนด spec ของ random forest

```{r}
rand_spec <- rand_forest(trees = 500, mtry = tune(), min_n = tune()) %>%
            set_engine("ranger") %>%
...
set.seed(1234)
my_folds <- vfold_cv(train_data, v = 5, strata = G3)
...
rf_best_fmeas <- rf_tune %>% select_best(metric = "f_meas")
```
...
```{r}
last_rf <- rf_wf %>%
finalize_workflow(rf_best_fmeas) %>%
last_fit(split,
        metrics = my_metrics)
```
...
![](https://miro.medium.com/v2/resize:fit:1400/format:webp/1*0qQTUDfImZYQBsyn9F6dpw.png)
