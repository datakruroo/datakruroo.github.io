...
```{r eval = F}
p <- parameters(penalty(range=c(-10,0)), 
                mixture(range=c(0,1)))
set.seed(1234)
regular_grid <- grid_regular(x = p, levels = 10)
random_grid <- grid_random(x = p, size = 20)
maxentropy_grid <- grid_max_entropy(x = p, size = 20)
latin_grid <- grid_latin_hypercube(x = p, size = 20)
...
p2 <- random_tune %>% collect_metrics() %>% 
  filter(.metric == "roc_auc") %>%
  ggplot(aes(x = penalty, y = mixture))+
  geom_tile(aes(fill = mean))+
  scale_x_log10()+
