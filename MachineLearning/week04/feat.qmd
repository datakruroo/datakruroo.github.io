...
```{r}
v <- vfold_cv(train, v=5, repeats = 3, strata = G3)

lm_resample <- lm_wf %>% 
  fit_resamples(resamples = v, control = control_resamples(save_pred = TRUE))

lm_resample %>% collect_metrics(summarize = F)
lm_resample %>% collect_predictions()
```
...
-   ในการสร้าง supervised learning model
